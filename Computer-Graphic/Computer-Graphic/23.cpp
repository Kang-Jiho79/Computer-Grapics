...
void renderViewport(int x, int y, int w, int h, const glm::mat4& viewMatrix, const glm::vec3& clearColor = glm::vec3(0.0f), bool projectioncheck = false)
{
	// 뷰포트 설정
	glViewport(x, y, w, h);

	// 시저 테스트 활성화 (뷰포트 영역만 클리어)
	glEnable(GL_SCISSOR_TEST);
	glScissor(x, y, w, h);

	// 배경색 설정 및 클리어 (해당 뷰포트 영역만)
	...
	// 축 그리기
	glm::mat4 axisMatrix = projection * viewMatrix * baseRotation;
	...
	glm::mat4 finalGroundMatrix = projection * viewMatrix * groundMatrix;
	...
	for (auto& shape : shapes) {
		...
		glm::mat4 finalMatrix = projection * viewMatrix * modelMatrix;
...
GLvoid drawScene()
{
	...
	// 뷰포트 1: 왼쪽 전체 - 메인 카메라 (원근 투영)
	glm::mat4 mainView = camera.getViewMatrix();
	renderViewport(0, 0, halfWidth, height, mainView, glm::vec3(0.1f, 0.1f, 0.1f), true);

	// 뷰포트 2: 오른쪽 위 - 탑뷰 (직교 투영)
	glm::mat4 topView = glm::lookAt(
		glm::vec3(0.0f, 1.0f, 0.0f),  // 위에서
		glm::vec3(0.0f, 0.0f, 0.0f),  // 원점을 바라봄
		glm::vec3(0.0f, 0.0f, -1.0f)  // Z축이 위쪽
	);
	renderViewport(halfWidth, halfHeight, halfWidth, halfHeight, topView, glm::vec3(0.0f, 0.1f, 0.1f), false);

	// 뷰포트 3: 오른쪽 아래 - 사이드뷰 (직교 투영)
	glm::mat4 sideView = glm::lookAt(
		glm::vec3(0.0f, 0.0f, 1.0f),  // 정면에서
		glm::vec3(0.0f, 0.0f, 0.0f),  // 원점을 바라봄
		glm::vec3(0.0f, 1.0f, 0.0f)   // Y축이 위쪽
	);
	renderViewport(halfWidth, 0, halfWidth, halfHeight, sideView, glm::vec3(0.1f, 0.0f, 0.1f), false);
