...
public:
	...
	std::vector<float> normals; // 법선 벡터 추가 (조명용)
	std::vector<int> index;
	float center[3]{};
	float size[3]{};
	float color[3]{};
	GLuint VAO, VBO[3], EBO; // VBO 배열 크기 3으로 증가 (정점, 색상, 법선)
...
// 조명 관련 변수들 추가
int currentLightColor = 0; // 조명 색상 인덱스 (0: 흰색, 1: 빨간색, 2: 파란색, 3: 초록색)
bool lighting = true; // 조명 켜기/끄기
bool lightRevolution = false; // 조명 공전 상태
bool showOrbit = true; // 궤도 표시/숨기기
float lightAngle = 0.0f; // 조명 각도
float lightRadius = 2.0f; // 조명 거리
...
std::string lightColorNames[4] = { "White", "Red", "Blue", "Green" };

// 조명 색상 변경 함수
void changeLightColor() {
	currentLightColor = (currentLightColor + 1) % 4;
	std::cout << "조명 색상: " << lightColorNames[currentLightColor] << std::endl;
}
...
void createOrbit(Shape& s, float radius, int segments) {
	...
	for (int i = 0; i <= segments; i++) {
		...
		s.vertices.push_back(1.0f); // y 위치를 1.0f로 (조명 높이와 맞춤)
...
void reset() {
	...
	lightCube.size[0] = 0.1f;  // 적당한 크기
	lightCube.size[1] = 0.1f;
	lightCube.size[2] = 0.1f;
...
void main(int argc, char** argv)
{
	...
	glutCreateWindow("Tank Animation with Lighting");
	...
	std::cout << "=== 조명 조작 ===" << std::endl;
	std::cout << "m: 조명 켜기/끄기" << std::endl;
	std::cout << "n: 조명 공전" << std::endl;
	std::cout << "b: 조명 색상 변경 (흰색→빨간색→파란색→초록색)" << std::endl;
	std::cout << "v/V: 조명 가까이/멀리" << std::endl;
	std::cout << "j: 조명 궤도 표시/숨기기" << std::endl;
...
{
	GLchar* vertexSource;
	vertexSource = filetobuf("vertex_light.glsl");  // 조명 셰이더 사용
...
void make_fragmentShaders()
{
	...
	fragmentSource = filetobuf("fragment_light.glsl");  // 조명 셰이더 사용
...
GLvoid initBuffer(Shape& shape)
{
	...
	glGenBuffers(3, shape.VBO); // 3개 버퍼 생성 (정점, 색상, 법선)
...
GLvoid drawScene()
{
	...
	// 조명 설정 (동적 색상 사용)
	glm::vec3 lightColor = lightColors[currentLightColor];
	glm::vec3 viewPos = camera.eye;  // 카메라 위치를 viewPos로 사용
	...
	glUniformMatrix4fv(modelMatLocation, 1, GL_FALSE, glm::value_ptr(groundMatrix));
	glBindVertexArray(ground.VAO);
	glDrawElements(GL_TRIANGLES, ground.index.size(), GL_UNSIGNED_INT, 0);

	// 탱크 부품들을 계층적으로 렌더링 (조명 적용)
	glUniform1i(lightingLocation, lighting);
	for (auto& shape : shapes) {
		...
		// 하부 몸체: 독립적으로 회전
		if (shape == &under_body) {
			rotate_Matrix(modelMatrix, glm::vec3(shape->center[0], shape->center[1], shape->center[2]),
				shape->x_rotationAngle, glm::vec3(1.0f, 0.0f, 0.0f));
			rotate_Matrix(modelMatrix, glm::vec3(shape->center[0], shape->center[1], shape->center[2]),
				shape->y_rotationAngle, glm::vec3(0.0f, 1.0f, 0.0f));
		}

		// 중앙 몸체 및 그 위의 모든 부품들: 중앙 몸체를 기준으로 회전
		else {
			// 먼저 중앙 몸체의 Y축 회전을 적용 (탱크 전체 회전)
			rotate_Matrix(modelMatrix, glm::vec3(mid_body.center[0], mid_body.center[1], mid_body.center[2]),
				mid_body.y_rotationAngle, glm::vec3(0.0f, 1.0f, 0.0f));

			// 개별 부품의 자체 회전
			rotate_Matrix(modelMatrix, glm::vec3(shape->center[0], shape->center[1], shape->center[2]),
				shape->x_rotationAngle, glm::vec3(1.0f, 0.0f, 0.0f));

			// 포신의 개별 회전 (상부 몸체를 기준으로)
			if (shape == &gun_barrel[0]) {
				rotate_Matrix(modelMatrix, glm::vec3(top_body[0].center[0] + top_body[0].animOffset[0],
					top_body[0].center[1], top_body[0].center[2]),
					shape->revolutionAngle, glm::vec3(0.0f, 1.0f, 0.0f));
			}
			else if (shape == &gun_barrel[1]) {
				rotate_Matrix(modelMatrix, glm::vec3(top_body[1].center[0] + top_body[1].animOffset[0],
					top_body[1].center[1], top_body[1].center[2]),
					shape->revolutionAngle, glm::vec3(0.0f, 1.0f, 0.0f));
			}

			// 깃대의 개별 회전 (상부 몸체를 기준으로)
			if (shape == &flag[0]) {
				rotate_Matrix(modelMatrix, glm::vec3(top_body[0].center[0] + top_body[0].animOffset[0],
					top_body[0].center[1], top_body[0].center[2]),
	...
	// 깊이 테스트 비활성화 (텍스트가 항상 위에 표시되도록)
	glDisable(GL_DEPTH_TEST);

	// 조명 색상에 맞게 텍스트 색상 설정
	glm::vec3 currentColor = lightColors[currentLightColor];
...
// Keyboard 함수에 조명 조작 키 추가
GLvoid Keyboard(unsigned char key, int x, int y)
{
	switch (key) {
		// 기존 탱크 조작 키들...
	case 't': // 중간 몸체 y축 회전
		addint(mid_body.y_rotate); // 중앙 몸체만 회전 제어
		break;
	case 'l': // 상부 몸체 위치 변경 (애니메이션)
		startSwapAnimation();
		break;
	case 'g': // 포신 y축 회전 (바깥쪽으로만)
		rotateGunBarrelOutward(0); // gun_barrel[0] 바깥쪽 회전
		rotateGunBarrelOutward(1); // gun_barrel[1] 바깥쪽 회전
		break;
	case 'p': // 깃대 x축 회전
		addint(flag[0].revolution);
		subint(flag[1].revolution);
		break;
	...
	case 'v': // 조명 가까이
		lightRadius -= 0.2f;
		if (lightRadius < 0.5f) lightRadius = 0.5f; // 최소 거리 제한
		updateLightOrbit();
...
{
	switch (key) {
	case GLUT_KEY_UP: // 위쪽 방향키 - z축 양방향 이동
		// 경계 체크
		if (shapes[0]->translation[2] + 0.1f < TANK_BOUNDARY) {
			for (auto& shape : shapes) {
				shape->translation[2] += 0.1f;
			}
		}
		break;
	case GLUT_KEY_DOWN: // 아래쪽 방향키 - z축 음방향 이동
		// 경계 체크
		if (shapes[0]->translation[2] - 0.1f > -TANK_BOUNDARY) {
			for (auto& shape : shapes) {
				shape->translation[2] -= 0.1f;
		...
		break;
	case GLUT_KEY_LEFT: // 왼쪽 방향키 - x축 양방향 이동
		// 경계 체크
		if (shapes[0]->translation[0] + 0.1f < TANK_BOUNDARY) {
			for (auto& shape : shapes) {
				shape->translation[0] += 0.1f;
			}
		}
		break;
	case GLUT_KEY_RIGHT: // 오른쪽 방향키 - x축 음방향 이동
		// 경계 체크
		if (shapes[0]->translation[0] - 0.1f > -TANK_BOUNDARY) {
			for (auto& shape : shapes) {
				shape->translation[0] -= 0.1f;
...
GLvoid Timer(int value)
{
	...
	// 상부 몸체 교환 애니메이션 처리
	if (isSwapping) {
		swapProgress += 0.016f / SWAP_DURATION; // 60fps 기준 (1/60 = 0.016초)
		...
		if (swapProgress >= 1.0f) {
			...
			// 애니메이션 오프셋 초기화
			top_body[0].animOffset[0] = 0.0f;
			top_body[1].animOffset[0] = 0.0f;
			gun_barrel[0].animOffset[0] = 0.0f;
			gun_barrel[1].animOffset[0] = 0.0f;
			flag[0].animOffset[0] = 0.0f;
			flag[1].animOffset[0] = 0.0f;

			std::cout << "상부 몸체 교환 애니메이션 완료!" << std::endl;
		}
		else {
			// 애니메이션 중 - 부드러운 easing 함수 사용 (ease-in-out)
			float t = swapProgress;
			float easedT = t * t * (3.0f - 2.0f * t); // smoothstep function

			// 상부 몸체 0은 오른쪽으로, 상부 몸체 1은 왼쪽으로 이동
			float targetDistance = originalPos1X - originalPos0X; // 0.4f (0.2 - (-0.2))

			top_body[0].animOffset[0] = targetDistance * easedT;
			top_body[1].animOffset[0] = -targetDistance * easedT;

			// 포신과 깃대도 함께 이동
			gun_barrel[0].animOffset[0] = top_body[0].animOffset[0];
			gun_barrel[1].animOffset[0] = top_body[1].animOffset[0];
			flag[0].animOffset[0] = top_body[0].animOffset[0];
			flag[1].animOffset[0] = top_body[1].animOffset[0];
		}
	}
	...
	for (auto& shape : shapes) {
		...
		// 포신 회전 처리 (바깥쪽으로만)
		if (shape == &gun_barrel[0] || shape == &gun_barrel[1]) {
			if (shape->revolution != 0) {
				int gunIndex = (shape == &gun_barrel[0]) ? 0 : 1;
				float currentTopBodyX = (gunIndex == 0) ?
					(top_body[0].center[0] + top_body[0].animOffset[0]) :
					(top_body[1].center[0] + top_body[1].animOffset[0]);

				float newAngle = shape->revolutionAngle + 2.0f * shape->revolution;

				// 바깥쪽 회전 제한 검사
				bool canRotate = false;
				if (currentTopBodyX < 0) {
					// 왼쪽 포신: 0도에서 -90도까지만 허용
					canRotate = (newAngle <= 0.0f && newAngle >= -90.0f);
				}
				else {
					// 오른쪽 포신: 0도에서 90도까지만 허용  
					canRotate = (newAngle >= 0.0f && newAngle <= 90.0f);
				}

				if (canRotate) {
					shape->revolutionAngle = newAngle;
				}
				else {
					// 한계에 도달하면 방향 반전 또는 정지
					if (currentTopBodyX < 0) {
						if (shape->revolutionAngle <= -90.0f) {
							shape->revolution = (shape->revolution == -1) ? 1 : 0;
						}
		...
		// 깃대 회전 (속도 감소 및 범위 축소)
		else if (shape == &flag[0] || shape == &flag[1]) {
			if (shape->revolution != 0) {
				// 깃대 회전 속도를 2.0f에서 1.0f로 감소
				shape->revolutionAngle += 1.0f * shape->revolution;

				// 회전 범위를 ±45도로 축소
				if (shape->revolutionAngle >= 45.0f)
					shape->revolution = -1;
				else if (shape->revolutionAngle <= -45.0f)
					shape->revolution = 1;
			}
		}
	}
	...
	// 중앙 몸체 Y축 회전 (별도 처리)
	if (mid_body.y_rotate != 0) {
		mid_body.y_rotationAngle += 2.0f * mid_body.y_rotate;
		if (mid_body.y_rotationAngle >= 360.0f)
			mid_body.y_rotationAngle -= 360.0f;
		else if (mid_body.y_rotationAngle < 0.0f)
			mid_body.y_rotationAngle += 360.0f;
	}
